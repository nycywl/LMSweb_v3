...
namespace LMSweb_v3.Services
{
    public class StudentManagementSercices
    {
        ...
        public List<StudentHomeViewModel> GetCoursesForStudent(string studentId)
        {
            var courses = _repository.Query<StudentCourse>()
                .Where(sc => sc.StudentId == studentId)
                .Select(sc => new StudentHomeViewModel
                {
                    CourseID = sc.Course.CourseId,
                    CourseName = sc.Course.CourseName,
                    TeacherName = sc.Course.Teacher != null ? sc.Course.Teacher.TeacherName : "未指定"
                }).ToList();
